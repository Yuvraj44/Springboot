...
public class AuthFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal( HttpServletRequest request,HttpServletResponse response,FilterChain filterChain) throws ServletException, IOException 
	{
		...
		userEmail = jwtServiceobj.extractUsername(jwt);
		
		//Step3: Update Security Context Holder
		if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) // user exists but authentication is not done
		{
			UserDetails user = this.userDetailsobj.loadUserByUsername(userEmail);
